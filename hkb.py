...
# Returns the rule used for a prediction (first rule if multiple rules are possible).
...
# Extracts all used rules from the inference output of InteKRator.
...
# Takes samples, performs inferences on the data and extracts the premises of the used rules.
...
# Converts cat features to a proper format for InteKRator.
...
# Converts numerical features to proper format for InteKRator, needed for inference.
...
# Converts list to a text file in the right data format for InteKRator, so the data can be read for inference.
...
# Checks if samples that are passed to InteKRator are in the right format. Technically this makes duplicate
# features possible (male 50 a_1 a_1 b_2 ...) but usually this should not happen, this method is mostly for making sure
# that the data passed is not totally off.
# The code skeleton for this method was created by an LLM, see thesis for details on the prompt and version.
...
# Prepares a given dataset for InteKRator, then calls InteKRator.jar as an external application to learn an HKB.
def fit(train_data, train_labels, name, cluster_size, kb, train_in="hkb_train_data.txt", preselect_value=19):
    while True:
        try:
            ...
            last_index = len(train_data_copy.columns)
            convert_cat_features(train_data_copy)
            train_labels_copy = train_labels.copy()
            diag_multi_col = train_labels_copy.pop("diag_multi")
            train_data_copy.insert(last_index, "diag_multi", diag_multi_col)
...
# Calls InteKRator.jar as an external application to infer from the HKB.
...
# Takes a dataset, feeds it into InteKRator to get an HKB and returns the predictions of the resulting HKB.
...
########################################################################################################################
# The following code show approaches to implement predict_proba for HKBs
########################################################################################################################
#
